/**
 * TradingGit AI Tools — unified wallet/git operations
 *
 * Replaces both createCryptoWalletToolsImpl and createSecWalletToolsImpl.
 * Tool names use sec* prefix to maintain backward compatibility with
 * existing MCP tool descriptions. These work for ALL account types.
 */

import { tool } from 'ai'
import { z } from 'zod'
import type { ITradingGit } from './interfaces.js'

export function createTradingGitTools(git: ITradingGit) {
  return {
...
IMPORTANT: This is READ-ONLY - it does NOT modify your actual holdings.
      ...
      execute: async ({ priceChanges }) => {
        return await git.simulatePriceChange(priceChanges)
      },
    }),
  }
}
